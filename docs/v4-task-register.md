...
Batch 1 = T-31, T-76, T-80, T-82, T-84 (PR #49). Batch 2 = T-81, T-83, T-78, T-77, T-85. Evidence for UI work comes from the global `visual-and-research` skill (Mobbin + Lenny's); each row names its reference.

| ID | Task | Acceptance |
|---|---|---|
| T-81 | **Copy apply** (PRD Part F). Apply the approved rows of the copy decision table (2026-08-30 audit; Abhishek's answer pending — default "all"). Text only: no logic, routes or scoring. Includes `lib/scoring/src/verdict.ts` sentence templates + `verdict-artefact.ts` + test fixture update, and a "lower is better" legend at the top of Results and the comparison section, with ↓/↑ on every numeric column. | Every string in the table reads as its rewrite; grep for the old terms (`provisional`, `evidence calls`, `Composite order`, `Leading, not decided`, `noise floor`) in `src/pages`, `src/components`, `lib/scoring`, `verdict-artefact.ts` returns nothing user-facing. |
...
| T-83 | **Public landing page** (Abhishek 2026-08-30: "B"). A separate static page (not behind the app shell) that explains what Transcribe Bench does for a client: the question it answers, the verdict artefact, how evidence is gathered, one call to action. Reference via `visual-and-research` (`search_sections`). Built with the app's tokens; no marketing claims the tool cannot back. | Route `/welcome` (or static `landing.html`) renders without the sidebar; copy reviewed by Abhishek. |
...
| T-85 | **Worst-first per-call ordering.** Results → "Compare providers per call" list and the Corpus table sort by peer disagreement, most-disagreed call first, so a reviewer opens the ugliest call first (Hamel & Shreya, Lenny's 2025-09-09: sort by score, look at the best and worst). Needs a per-call disagreement number in the manifest or calls list. | List order matches the disagreement count; visible label "most disagreement first". |

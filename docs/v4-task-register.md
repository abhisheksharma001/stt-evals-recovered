...
| T-80 | **Raw Tailwind colours leaking past the semantic tokens** (found during T-70, D.5 rule): `components/judge-accuracy-card.tsx:129,131` (`text-emerald-700`/`bg-emerald-500/15`, `text-red-700`/`bg-red-500/15`) and `pages/Bulks.tsx:207` (`text-amber-700`). Replace with `success`/`destructive`/`warning` tokens. | todo — 10 minutes; no behaviour change. |

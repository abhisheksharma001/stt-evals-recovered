...
| T-43 | **The retry gate is UI-only; the executor still re-runs permanently-dead cells.** T-07 disables the button when nothing is retryable, but `executeBenchmarkRun`'s only skip condition is `alreadyOk` (`lib/run-executor.ts`) — so on a **mixed** bulk (some retryable failures, some not) the button is correctly enabled and the retry then re-bills the `retention_expired` / `audio_url_forbidden` cells too, which can never succeed. The server must refuse them itself: skip failed cells whose class is non-retryable, and whose class is null, leaving their existing rows untouched. Deliberately not folded into T-07 — that PR is a read path plus a UI gate, this one changes what the executor spends money on, and the two should not be reviewed as one diff. | T-07 self-review |

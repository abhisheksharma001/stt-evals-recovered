...
| T-45 | **A no-op retry still pays OpenAI.** `runAutoAgentVerificationForRun` re-verifies every `ok` cell of the run on **every** re-execution, with no per-(run, call) dedupe — so re-executing a run that has nothing left to do (now the normal outcome for a bulk whose remaining failures are all permanent, after T-43) still fires the judge for every already-verified call and is billed for it. Pre-existing, not introduced by T-43, but T-43 makes hitting it routine. Fix is a guard on an existing scan row for that (callId, runId), not a change to the judge. | T-43 self-review |
| T-46 | `retryBulkFailedCells` picks a run for retry if it has **any** `failed` or `skipped_pending_review` row, without looking at the failure class — so a run whose every failure is permanent is still re-executed. After T-43 that re-execution spends nothing on providers, but it flips the bulk back to `running`, clears `completedAt`, rewrites the run status, and triggers T-45's OpenAI spend. The selection query should apply the same retryability rule the executor now applies. | T-43 self-review |

...
| T-68 | With install fixed, `deploy-web.yml` now runs to its Vercel step on every main push touching UI paths and fails there: no `VERCEL_TOKEN` / `VERCEL_ORG_ID` / `VERCEL_PROJECT_ID` secrets exist. Nothing is deployed to Vercel today and nothing in the loop depends on it. Either add the secrets (a deliberate decision to host the UI there) or disable the workflow so main does not carry a permanent red X. | Decide |

...
| T-53 | **GitHub CI has been red on every run since PR #20, `main` included, before any test runs**: `.github/workflows/ci.yml` and `deploy-web.yml` pin `pnpm/action-setup` to `version: 9`, but `pnpm-lock.yaml` is written by pnpm 11 (local `pnpm --version` = 11.1.2), so `pnpm install --frozen-lockfile` fails with `ERR_PNPM_LOCKFILE_CONFIG_MISMATCH ... "overrides" configuration doesn't match the value found in the lockfile`. Nothing in the loop depends on CI today (every PR is typechecked, tested, and deployed locally), but `gh pr merge` refuses an UNSTABLE PR — #22 was merged through the REST merge endpoint (same as the UI button, no protection bypass). Fix is one line: pin the action to the pnpm major the lockfile was written with (or read `packageManager` from `package.json`). Also check `deploy-web.yml`, which has the same pin and presumably fails the same way. | T-14 |
| T-54 | **OpenAI 429s get no back-off in the agent path.** `callOpenAi` (`lib/agent.ts`) throws `AgentRequestError` on any non-2xx with no retry, so a rate-limited judge call lands as a `status: "error"` scan row (redone on the next execution, per T-45). With T-15 running 4 judge calls per shard × `BULK_SHARD_CONCURRENCY` 3 shards = up to 12 in flight, 429s are likelier than before. PRD-v4-technical §V4-T15 asks for the same halve-and-back-off providers get (`run-executor.ts` T-6 fix); the register row for T-15 did not include it. Not seen in the live probe (16 calls, zero errors). | T-15 |
| T-55 | **Per-assistant grouping starves the verdict.** Found live in T-20: each weekly bulk fragments into 22–23 assistant groups, 20+ of them 1–2 calls, so no group can ever reach the 5-shared-call floor, let alone the 20-call decision bar — the verdict will say `too_few_calls` for every real client group forever at current volume. Rankings group the same way. Options to decide (Abhishek's call, not a drive-by): verdict at bulk level (all calls, one verdict) with per-assistant as a drill-down; or group by vertical again for the verdict only; or a bulk-level verdict only when `assistantId` groups are all below the floor. T-21 (verdict UI) should not ship on top of 22 "no winner" cards. | T-20 live data, 2026-08-29 |
| T-56 | **`vsProductionPct` can never fill on the current bulks.** Production for every group is `deepgram / flux-general-en` (or `nova-2`), and the provider rows exist (`deepgram-flux-general-en`, `deepgram-nova-2`) but no bulk selects them, so the verdict's vs-production comparison is always null and U-12's "22% cleaner than production" line has nothing to say. Either add the production provider to the bulk templates by default (costs real money per bulk) or have the preview warn when production isn't among the candidates. | T-20 live data, 2026-08-29 |
| T-57 | **Same card contradicts itself: verdict vs. composite "Recommended".** Seen live after T-21: a group headline says "Too close to call: Deepgram and AssemblyAI are within the noise floor" while, directly under it, row 1 carries the trophy + "Recommended" badge and the Decision Logic footer says "Leading candidate … Do not treat as decision-grade." Two verdict systems on one card (T-20 noise-floor verdict vs. old composite rank recommendation). Decide which one owns the word "Recommended": either the badge/footer defer to the T-20 decision (badge only when decision = winner, otherwise "Leading, not decided"), or the recommendation text is rewritten to cite the verdict. | T-21 live, 2026-08-29 |
| T-58 | Latency column prints unrounded floats (`17189.889ms`, `124769.11ms`, `2973.3333ms`) next to integer values (`9509ms`). Format to whole ms. Cosmetic, pre-existing. | T-21 live, 2026-08-29 |
| T-59 | **"Couldn't play audio" toast blames the wrong thing.** `SpanAdjudicator.play()` catches every `audio.play()` rejection with "The recording isn't cached on the server and its Vapi link may have expired." Seen in T-22 verification: the audio endpoint returned 200 `audio/wav` 3.6MB, yet the toast fired — the rejection was `NotAllowedError` (browser autoplay policy, no prior user gesture on the document). Branch on `err.name`: `NotAllowedError` → "Click anywhere on the page first, then play"; `NotSupportedError`/network → the cache/expiry copy. Cosmetic-but-misleading. | T-22 live, 2026-08-29 |
| T-60 | `fetchVapiCalls` (`lib/vapi.ts`) paging silently stops after one page when Vapi returns newest-first: cursor = newest `createdAt` on the page → next page has 0 fresh → break. Seen live 2026-08-29: same 14-day window returned exactly 1,000 calls via `fetchVapiCalls` and 3,448 via descending paging. Any import with `limit > 1000` undercounts without saying so. Fix: page on the oldest `createdAt` (as `lib/volume.ts` now does) or set `truncated`. | todo |
| T-61 | Ranking rows' `score.costPerMinute` is not a per-minute price: for one bulk it reads 0.0036–0.0348 for AssemblyAI across groups (list $0.006), 0.0013–0.0127 for Cartesia — it moves with call length, so it is a per-cell average of something else. The Cost/Min column and the baseline sentence's "$/min cheaper" both read it. T-24 deliberately used catalog list prices instead. Decide what the cell metric is and label or fix the column. | todo |
| T-62 | Catalog list price for `deepgram-flux-general-en` equals Nova-3's ($0.0043) — likely copied, not verified. Flux is what production runs, so the T-24 "cost alone" sentence inherits it. Verify against Deepgram's price list before anyone quotes it. | todo |
| T-63 | The 10 historical `flagged` scans with a null pick (pre-BAML, strict-schema era) stay null forever: T-45 only redoes `error`/`scanning` scans on re-execute, and a `flagged` scan with a null pick counts as finished. Their per-call disagreement views (T-22) and judge-accuracy replay have no pick to show. Options: a one-off re-judge of those 10 (≈ $0.08 on gpt-4o) or teach T-45 to treat `flagged` + null pick as "not finished". Found while verifying T-25. | Decide; either is small. |
| T-64 | BAML `linux-x64-gnu` native pin is in the lockfile but has never been exercised: CI is permanently red (T-53) and the only deploy target is this Mac. First Linux install/build will be the first real test of the pin. Pairs with T-53. | None until CI or a Linux host exists. |
